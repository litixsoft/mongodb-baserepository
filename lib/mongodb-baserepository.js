...
module.exports = function (collection, schema) {
    ...
    function analyseSchemaProperty (name, prop) {
        ...
        if (lxHelpers.isArray(prop.type)) {
            lxHelpers.forEach(prop.type, function (type) {
                if (type === 'string' && prop.format === 'mongo-id') {
                    if (!idFields.hasOwnProperty(name)) {
                        idFields[name] = true;
                    }
                }
            });
        } else if (prop.type === 'string' && prop.format === 'mongo-id') {
            if (!idFields.hasOwnProperty(name)) {
                idFields[name] = true;
            }
        }
    ...
    /**
     * Exports the Object which is used as an id in the repository
     */
    pub.idObject = ObjectID;
     ...
     *
     * @returns {{unknownProperties: string, convert: Function, trim: boolean, strictRequired: boolean}}
     */
    ...
    /**
     * Returns the given id as an mongo ObjectID.
     *
     * @param {String|ObjectID} id The id to convert.
     * @returns {ObjectID}
     */
    pub.ensureObjectID = function (id) {
        if (lxHelpers.isString(id)) {
            id = ObjectID.createFromHexString(id);
        }
    ...
    pub.findOne = function (query, options, callback) {
        ...
        query = convertToMongoId(query);
        collection.find(query, mongoOptions).limit(1).next(callback);
    };
     ...
     * @param {!function(err, res)} callback The callback.
     */
    pub.findOneAndUpdate = function (filter, update, options, callback) {
        filter = convertToMongoId(filter);
    ...
    /**
     * Deletes a single document on MongoDB, returning the deleted document.
     *
     * @param {!Object} filter The filter object.
     * @param {Object=} options Optional settings.
     * @param {!function(err, res)} callback The callback.
     */
    pub.findOneAndDelete = function (filter, options, callback) {
        filter = convertToMongoId(filter);
    ...
    /**
     * Modifies and replaces a single document on MongoDB.
     *
     * @param {!Object} filter The filter object.
     * @param {!Object} replacement The replacement data.
     * @param {Object=} options Optional settings.
     * @param {!function(err, res)} callback The callback.
     */
    pub.findOneAndReplace = function (filter, replacement, options, callback) {
        filter = convertToMongoId(filter);

        // delete key property
        deleteDocumentKey(replacement);

        options = options || {};
        collection.findOneAndReplace(filter, replacement, options, callback);
    };
